...
###################################################
def pyproject_read(gitRoot: str) -> dict | None:
    ...
    if pyproject.exists():
        ...
        logger.error("file 'pyproject.toml' NOT found.")
    return None
...
# ---- update pyproject.toml ----
###################################################
def pyproject_write(gitRoot: str, data: dict, fExecute: bool) -> None:
    pyproject = Path(gitRoot) / "pyproject.toml"
    if pyproject.parent.exists():
        import tomli_w
        if fExecute:
...
# ---- update pyproject.toml ----
###################################################
def pyproject_version(new_version: str, fExecute: bool, gitRoot: str):
    pyproject = Path(gitRoot) / "pyproject.toml"
